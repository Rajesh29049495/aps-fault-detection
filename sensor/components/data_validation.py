...
class DataValidation:
    ...
    def __init__(self,
                    ...
                    data_ingestion_artifact:artifact_entity.DataIngestionArtifact):  ##this DataIngestionArtifact class we have provided as an input because we wanted to use the path values wheer we have stored the train data and test data{that we got out of the first component}
        try:
    ...
    def drop_missing_values_columns(self,df:pd.DataFrame,report_key_name:str)->Optional[pd.DataFrame]:  ##as it may return or may not return anything, thats why Optional is used{remember to import it},,,,,,,,,,and the report_key _name variable is basically a key of the validation_error dictionary and to this key we will have some report here in this function which later on as awhole will be used to form the whole report w.r.t. the data validation functions done in this file
        """
        ...
        try:
            ...
            null_report = df.isna().sum()/df.shape[0]  ##this will store percentage of null values in each column
            #selecting column name which contains null
            logging.info(f"selecting column name which contains null above to {threshold}")
            drop_column_names = null_report[null_report>threshold].index  ##it will save all the columns with null values greater than threshold values in drop_column_names
    ...
    def is_required_columns_exists(self,base_df:pd.DataFrame,current_df:pd.DataFrame,report_key_name:str)->bool:  ##this function will return a boolean value after checking no of columns in both the dataframe
        try:
...
##we performed three data validation functions, apart from it any other data validation operation you wanna perform you can perform
    ...
    def initiate_data_validation(self)->artifact_entity.DataValidationArtifact:
        try:
            ...
            ##if both the status of above called "is_required_columns_exists" function is True then we call "data_drift" function 
