...
##now we will define the transformer object using which transformation will be done
...
def initiate_data_trasnformation(self) -> artifact_entity.DataTransformationArtifact:
    try:
        ...
        target_feature_train_arr = label_encoder.transform(train_df[TARGET_COLUMN])
        target_feature_test_arr = label_encoder.transform(test_df[TARGET_COLUMN])
