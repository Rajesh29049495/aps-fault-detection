...
class ModelTrainer:
    ...
    # function to train the model using the XGBClassifier
    def train_model(self,x,y):
        try:
            xgb_clf =  XGBClassifier()      ###training the model by using this classifier ,because during th eexperiment stage found this algorithm to be the best for the problem
            xgb_clf.fit(x,y)
            return xgb_clf
        except Exception as e:
            raise SensorException(e, sys)
    ...
    def initiate_model_trainer(self,)->artifact_entity.ModelTrainerArtifact:
        try:
            ...
            ##calculating the accuracy score, for that we will be using the f1 score, because we wanted to giv eimportane to both precision and recall hence we used the f1 score as it i steh harmonic mean of both,,,,,,,note in case of regeression problem we would have used r1 score
